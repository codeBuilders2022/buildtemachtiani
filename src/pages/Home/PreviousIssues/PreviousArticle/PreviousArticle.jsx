...
const PreviousArticle = () => {
  ...
  const getDatas = async () => {
    const idArticle = Decrypt(ida)
    try {
      // Hacemos una llamada concurrente a la API utilizando Promise.all()
      const resCommittees = await getAxiosHomeArticles(`/api/previous-articles/${idArticle}?populate=*`)
      const data = resCommittees.data.attributes
      setPdf(process.env.REACT_APP_API_URL + resCommittees?.data?.attributes?.pdf?.data?.attributes?.url)
  ...
  return (
    <div className="PreviousArticle dark:bg-gray-600 dark:text-white m-2 md:m-10 md:mt-32 mt-24 p-2 md:p-10 bg-white rounded-3xl flex">
      <Back className={"_back_l"} url={`/previous-issues/${id}`} />
      <div style={{ width: '100%' }}>
        <div className="type">
          <p>{dataArt?.type}</p>
        </div>
